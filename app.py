...
def format_owners(owners):
    return ", ".join([o.get("displayName", "Unknown") if isinstance(o, dict) else str(o) for o in owners])
    ...
    try:
        ...
        for team in league.teams:
            teams_data.append({
                "id": team.team_id,
                "name": team.team_name,
                "owners": format_owners(team.owners),
                "wins": team.wins,
                "losses": team.losses,
...
def standings():
    try:
        league = get_league()
        data = [{"name": t.team_name, "owners": format_owners(t.owners), "wins": t.wins, "losses": t.losses} for t in sorted(league.teams, key=lambda t: t.wins, reverse=True)]
...
def dashboard():
    try:
        ...
        for team in league.teams:
            ...
            teams_data.append(t)
            if "slumber" in team.team_name.lower() or "matt" in format_owners(team.owners).lower():
                my_team = t
        box_scores = league.box_scores(week)
        matchups = [{"home": m.home_team.team_name, "homeScore": m.home_score, "away": m.away_team.team_name, "awayScore": m.away_score} for m in box_scores]
